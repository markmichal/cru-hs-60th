...
>
> **Latest (2026-06-27, no owner action):** the share.html media input was overhauled (auto-detect pasted links, either/or one-media-per-story), **all videos now embed in-site including Vimeo**, and a follow-up fixed **Vimeo input thumbnails** (via Vimeo's v2 JSONP API — oEmbed is CORS-blocked) plus added a **media preview to the verify step**. All pure front-end, goes live on the Pages rebuild. See "2026-06-27 — media input overhaul + Vimeo embedding" and its follow-up.
>
...
- **Why not oEmbed as asked:** oEmbed can't be reached from the browser without a server proxy (would need an Apps Script action — out of scope/"no server change"). The v2 JSONP API is the working client-side equivalent and returns the same thumbnail, so no proxy was added.
